...
static void rcu_momentary_dyntick_idle(void)
{
	 ...
	 */
	resched_mask = rcu_sched_qs_mask[get_cpu()];
	rcu_sched_qs_mask[get_cpu()] = 0;
...
void rcu_note_context_switch(void)
{
	...
	if (unlikely(rcu_sched_qs_mask[get_cpu()]))
			      ...
			      struct rcu_data *rdp)
{
	...
	if (rdp->gpnum != rnp->gpnum || unlikely(READ_ONCE(rdp->gpwrap))) {
		...
		rdp->rcu_qs_ctr_snap = rcu_qs_ctr[get_cpu()];
...
static void rcu_cleanup_dying_cpu(struct rcu_state *rsp)
{
	...
	RCU_TRACE(struct rcu_data *rdp = &rsp->rda[get_cpu()]);
...
static int rcu_pending(void)
{
	...
	for_each_rcu_flavor(rsp)
		if (__rcu_pending(rsp, &rsp->rda[get_cpu()]))
			return 1;
	return 0;
}
